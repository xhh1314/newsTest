package com.hww.ucenter.webadmin.util;
...
import com.hww.ucenter.webadmin.service.impl.SysAuthServiceImpl;
...
public class SiteIdUtil {
	...
	@Autowired
	static SysAuthServiceImpl sysAuthServiceImpl;
	...
	public static SysSiteDto getByParams(HttpServletRequest request, HttpServletResponse response) {
		...
		if (!StringUtils.isBlank(p)) {
			try {
				...
				site = sysAuthServiceImpl.findSiteById(siteId);
				if (site != null) {
	...
	public static SysSiteDto getByCookie(HttpServletRequest request) {
		...
		if (cookie != null) {
			...
			if (!StringUtils.isBlank(v)) {
				try {
					Integer siteId = Integer.parseInt(v);
					return sysAuthServiceImpl.findSiteById(siteId);
				} catch (NumberFormatException e) {
					log.warn("cookie site id format exception", e);
				}
			}
	...
	public static SysSiteDto getByDomain(HttpServletRequest request) {
		...
		if (!StringUtils.isBlank(domain)) {
			...
			List<SysSiteDto> p = sysAuthServiceImpl.list(search);
			if (p != null && p.size() > 0) {
	...
	public static SysSiteDto getByDefault() {
		List<SysSiteDto> p = sysAuthServiceImpl.list(null);
		if (p != null && p.size() > 0) {
			return (SysSiteDto) p.get(0);
		} else {
			return null;
