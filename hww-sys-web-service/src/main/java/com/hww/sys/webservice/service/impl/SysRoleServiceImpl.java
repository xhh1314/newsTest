...
public class SysRoleServiceImpl implements SysRoleService {
	...
	public SysRoleDto findRoleWithMenuById(Long id) {
		...
		/*
		 * for (SysMenu menu : entity.getSysMenus()) { menuIds.append(menu.getMenuId());
		 * menuIds.append(","); }
		 */
		SysRoleDto dto = BeanMapper.map(entity, SysRoleDto.class);
	...
	public List<SysRoleDto> allList() {
		...
		for(SysRole role:list) {
			SysRoleDto dto=BeanMapper.map(role, SysRoleDto.class);
			result.add(dto);
		}
	...
	public List<SysRoleDto> findByUserId(Long userId) {
		List<SysUserRRole> rolelist = sysUserRRoleMng.findByUserId(userId);
		
		List<SysRole> dlist=new ArrayList<SysRole>();
		
		for(SysUserRRole r:rolelist) {
			Long roleID=r.getRoleId();
			SysRole role=sysRoleMng.find(roleID);
			if(role!=null&&role.getStatus()!=null&&role.getStatus().intValue()==1) {
				dlist.add(role);
			}
		}
		
		List<SysRoleDto> dtlist=BeanMapper.mapList(dlist, SysRoleDto.class);
		return dtlist;
	}
