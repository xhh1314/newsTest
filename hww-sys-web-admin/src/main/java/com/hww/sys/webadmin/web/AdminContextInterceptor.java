...
public class AdminContextInterceptor
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 获得站点
		Long oldSiteId = getSiteIdByCookie(request);
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		/*
		 * SysUser user = SysUtils.getUser(request); SysSite site =
		 * SysUtils.getSite(request); //
		 * 不控制权限时perm为null，PermistionDirective标签将以此作为依据不处理权限问题。 if (auth && user
		 * != null && !user.isSuper() && mav != null && mav.getModelMap() !=
		 * null && mav.getViewName() != null &&
		 * !mav.getViewName().startsWith("redirect:")) {
		 * mav.getModelMap().addAttribute(PERMISSION_MODEL,
		 * getUserPermission(site, user)); }
		 */
	}
	...
	private void setSite(HttpServletRequest request, HttpServletResponse response, Long oldSiteId, SysSiteDto site) {
	...
	private Long getSiteIdByCookie(HttpServletRequest request) {
