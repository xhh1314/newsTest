...
public class SnsPostController {
	...
	public R upPost(HSnsPostUpParamDto postUpParamDto) {
		...
		try {
			String isExists = appFeignClient
					.behaviorExist(new AppBehaviorDto(postUpParamDto.getMemberId(), postUpParamDto.getPostId(),
							AppBehaviorDto.BevType.b1_dz).setPlateType(AppBehaviorDto.PlatType.b2_post));
			if ("y".equals(isExists)) {
				return R.error("已经点过赞，请不要重复操作");
			}
			AppBehaviorDto behaviorDto = new AppBehaviorDto(postUpParamDto.getMemberId(), postUpParamDto.getPostId(),
					AppBehaviorDto.BevType.b1_dz, AppBehaviorDto.BevValue.b1_ok, AppBehaviorDto.PlatType.b2_post);
	...
	public R cancelUpPost(HSnsPostUpParamDto postUpParamDto) {
		...
		try {
			AppBehaviorDto behaviorDto = new AppBehaviorDto(postUpParamDto.getMemberId(), postUpParamDto.getPostId(),
					AppBehaviorDto.BevType.b1_dz, AppBehaviorDto.BevValue.b0_cancel, AppBehaviorDto.PlatType.b2_post);
			return appFeignClient.createBehavior(behaviorDto);
		} catch (HServiceLogicException e) {
			return R.error(1, e.getMessage());
		} catch (Exception e) {
	...
	@RequestMapping(value = "getSnsPostFromDatabase.do", method = RequestMethod.GET)
	public SnsPost getSnsPostFromDatabase(@RequestParam("postId") Long postId) {
		return snsPostMng.getSnsPostFromDatabase(postId);
	...
	@RequestMapping(value = "updateLikesAndCommentNum.do", method = RequestMethod.POST)
	public R updateLikesAndCommentNum(@RequestParam("postId") Long postId, @RequestParam("upNum") Integer upNum,
			@RequestParam("commentNum") Integer commentNum) {
		snsPostMng.updateLikesAndCommentNumToDataBase(postId, upNum, commentNum);
		return R.ok();
	}
	// ==================FeginApi================end==================================================
	// @RequestMapping(value = "loadNewsPostList.do", method = RequestMethod.POST)
	...
	//
	// try {
	// List<SnsPostVo> topicPostList =
	// snsPostService.loadPostListByNewsId(snsqueryDto);
	//
	// return R.ok().put("data", topicPostList);
	//
	// } catch (HServiceLogicException e) {
